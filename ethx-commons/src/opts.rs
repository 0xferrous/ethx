use crate::{WalletSigner, tempo::TempoAccessKeyConfig, utils, wallet_raw::RawWalletOpts};
...
pub struct WalletOpts {
    ...
    /// Raw wallet input options such as private keys and mnemonics.
    #[command(flatten)]
    ...
    #[arg(long, help_heading = "Wallet options - remote", hide = true)]
    pub aws: bool,
...
impl WalletOpts {
    ...
    ///
    /// Returns the signer and an optional Tempo access-key configuration.
    /// When no wallet configuration is present, this returns `(None, None)`.
    ...
    /// Resolves a signer or returns a user-facing error describing supported inputs.
...
mod tests {
    ...
    async fn illformed_private_key_generates_user_friendly_error() {
        ...
        match wallet.signer().await {
            Ok(_) => panic!("illformed private key shouldn't decode"),
            Err(x) => {
                assert!(
